...
DATASETS_SELF_LOOP = [PurePath(RAPIDS_DATASET_ROOT_DIR)/f for
                       f in ["ca-AstroPh.txt","ca-CondMat.txt","ca-HepTh.txt","ca-GrQc.txt "]]

#DATASETS_ISLT_STR = [PurePath(RAPIDS_DATASET_ROOT_DIR)/f for
#                      f in ["karate_mod.txt"]]

DATASETS_STR = [PurePath(RAPIDS_DATASET_ROOT_DIR)/f for 
                       f in ["karate_str.mtx", "karate_mod.mtx"]]
    ...
    if read_weights:
        if read_weights_in_sp is True:
            df = pd.read_csv(
                ...
                header=None,
                names=["0", "1", "weight"],
                dtype={"0": str, "1": str, "weight": "float32"},
            )
        else:
            df = pd.read_csv(
                csv_file,
                ...
                header=None,
                names=["0", "1", "weight"],
                dtype={"0": str, "1": str, "weight": "float64"},
            )
    else:
        df = pd.read_csv(
            csv_file,
